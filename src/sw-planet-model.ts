//#region Hardcoded Planets

//
// Common to use hardcoded data like this to build SPA...
//
// And trust we'll be able to load "real" data in future : - )
//
...
export type SwapiPlanet = typeof allPlanets[0];

export type PlanetWithFilmCount = {
	name: string;
	filmCount: number;
};

export type PlanetWithFormattedPopulation = {
	name: string;
...
export const getPlanetsByClimate = (
	planets: SwapiPlanet[],
	lookupClimate: string,
): string[] => {

	const planetsForFiltering: PlanetForFiltering[] = planets
		.filter(
			x => x.name !== "unknown"
		)
		.map(
	...
	return planetsForFiltering
		.filter(
			// x => x.arrayOfClimates.some(
			// 	y => y === lookupClimate
			// )

			// Well actually, from A. Spriggs...
			x => x.arrayOfClimates.includes(lookupClimate)
		)
		.map(
			x => x.name
		)
