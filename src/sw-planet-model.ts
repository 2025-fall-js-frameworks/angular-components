...
// Exported functions...
//
export const getClimates = (ppp: SwapiPlanet[]): string[] =>
	ppp
		.map((x) => x.climate)
		.flatMap((x) => x.split(','))
		.map((x) => x.trim())
...
export const getPlanetsByClimate = (planets: SwapiPlanet[], lookupClimate: string): string[] => {
	const planetsForFiltering: PlanetForFiltering[] = planets
		.filter((x) => x.name !== 'unknown')
		.map((x) => ({
			name: x.name,
			arrayOfClimates: x.climate.split(',').map((y) => y.trim()),
		}));
...
export const getPlanetsWithFilmCount = (planets: SwapiPlanet[]): PlanetWithFilmCount[] =>
	planets
		.map((x) => ({
			name: x.name,
			filmCount: x.films.length,
		}))
		.filter((x) => x.filmCount > 0)
...
const foo = allPlanets.filter((x) => true).filter((x) => true);
